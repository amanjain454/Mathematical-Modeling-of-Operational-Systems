...
param revenue {material,1..T} >= 0;# Revenue from selling material M to demand in time T when supplied at Ts
...
maximize total_profit:
      sum {t in 1..T} ( sum {mat in material} (sum {ts in t..Ts} revenue [mat,t] * demand_served [mat,t,ts])
...
s.t. material_balance_product_1 {j in prod, t in 2..T}:
      sum {i in prod} aij[i,j] * made[i,t] + inv[j,t] + sum {ts in 1..t} demand_served [j,t,ts] >=  made[j,t] + inv[j,t-1] ;
...
s.t. material_balance_raw_1 {j in raw, t in 2..T}:
     inv[j,t] + sum {ts in 1..t} demand_served [j,t,ts] >= supply[j,t] + inv[j,t-1] ;
      # scr[j,t] + inv[j,t] + sum {ts in 1..t} demand_served [j,t,ts] = supply[j,t] + inv[j,t-1] ; 
#constraint 3 : Material balance for product for period equal 1. 

s.t. material_balance_product_2 {j in prod}:
      sum {i in prod} (aij[i,j] * made[i,1]) + inv[j,1] + demand_served [j,1,1] >= made[j,1] ;
...
s.t. material_balance_raw_2 {j in raw}:
     inv[j,1] + demand_served [j,1,1] >= supply[j,1] ;
...
s.t. cap_balance {j in cap, t in 1..T}:
      sum {i in prod} aij[i,j]*made[i,t]  >= supply[j,t] ;
...
s.t. raw_balance_1 {j in raw, t in 2..T}:
      sum {i in prod} (aij[i,j] * made[i,t]) + inv[j,t] + sum {ts in 1..t} demand_served [j,t,ts] >= supply[j,t] + inv[j,t-1] ;
